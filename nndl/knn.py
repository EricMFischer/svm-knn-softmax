  ...
  def compute_distances(self, X, norm=None):
    ...
    for i in np.arange(num_test):
      ...
      for j in np.arange(num_train):
        ...
        train = self.X_train[j] # (3072,)
        test = X[i] # (3072,)
        dists[i,j] = norm(train - test)
...
#     a2 = np.square(X)
#     b2 = np.square(self.X_train)
#     ab = X.dot(self.X_train.T)
    a2 = np.sum(X**2, axis=1).reshape(num_test, 1) # (500,1)
    b2 = np.sum(self.X_train**2, axis=1).reshape(1, num_train) # (1,5000)
